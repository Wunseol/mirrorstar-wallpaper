...
> 本文档整合 `audio`（WASAPI 音量控制）、`ipc`（命名管道 IPC 协议）、`process`（子进程管理）三个紧密协作的核心库子模块，共同支撑视频壁纸（mpv）与网页壁纸（wp-proc）的进程级控制链路。
> 本节"现状"与文件清单源自 **v6.0 技术债审查**（audio/ipc/process 模块），反映了代码当前状态。
...
| 文件 | 行数 | 主要内容 |
|---|---|---|
| `crates/mirrorstar-core/src/audio/mod.rs` | 1 | 仅 `pub mod volume;` |
| `crates/mirrorstar-core/src/audio/volume.rs` | 1030 | `VolumeControl` 实现 + `is_pid_running` / `collect_stale_pids` 辅助 + 19 个测试 |

#### ipc 子模块

| 文件 | 行数 | 主要内容 |
|---|---|---|
| `crates/mirrorstar-core/src/ipc/mod.rs` | 32 | 模块级 `//!` 文档（含超时对照表 I-006）+ 子模块声明 + `pub use NamedPipeIpcClient` |
| `crates/mirrorstar-core/src/ipc/client.rs` | 1086 | `NamedPipeClient<T>` 泛型基类 + `NamedPipeIpcClient` trait + `connect_named_pipe` / `read_line_with_limit` / `read_line_with_timeout` / `Backoff` + 30+ 测试 |
| `crates/mirrorstar-core/src/ipc/mpv_protocol.rs` | 414 | `MpvIpcClient` + `MpvCommand` / `MpvResponse` / `MpvEvent` + 14 测试 |
| `crates/mirrorstar-core/src/ipc/wp_proc.rs` | 713 | `WpProcIpcClient` + `WpProcCommand` / `WpProcResponse` / `ResponseStatus` + 18 测试（含 I04 真实管道超时测试） |

#### process 子模块

| 文件 | 行数 | 主要内容 |
|---|---|---|
| `crates/mirrorstar-core/src/process/mod.rs` | 1 | 仅 `pub mod manager;` |
| `crates/mirrorstar-core/src/process/manager.rs` | 1156 | `ProcessManager` 实现 + `escape_windows_arg` MSVCRT 转义 + `wait_and_terminate` / `terminate_and_wait` 共享逻辑 + 18 测试 |
...
> **状态汇总（已对照真实代码核验，2026-09 更新）**：22 项中 **21 项已修复（✅）**，仅 **A-001 仍未完成**（⚠️ 部分缓解，主动监听未实现；详见各条「修复状态」及文末 §3.2 交集汇总）。

### 审查重点说明

三个模块经过 v3.0→v3.5→v4.0→v4.1 多轮修复后整体质量较高：audio 的缓存失效防护（C-103/C-026/A03）与优雅降级（T-008）及 v41-A-001 COM 自愈；ipc 的 OOM 防护（I02）、指数退避（C-113）与 I-001 `total_avail` 超时盲区修复；process 的 STILL_ACTIVE 误判修复（P01）、`stop_immediate` 立即终止路径（v41-P-002）与 Job Object 配置。v4.0 的三项 High 优先项中 **I-001、P-001 已修复**，仅 **A-001（音频设备主动变更监听）仍未实现**。

### audio 模块 findings（6 项）

#### [A-001] [High] [架构设计] volume.rs:92, 14 — `refresh_session_manager` 无生产调用方，音频设备变更完全无感知 ⚠️ 部分缓解（主动监听未实现）

**描述**：`VolumeControl` 持有 `device_enumerator: Option<IMMDeviceEnumerator>`（L14），其唯一 post-construction 消费者是 `refresh_session_manager`（L92-103）。模块未实现 `IMMNotificationClient`（`IMMDeviceEnumerator::RegisterEndpointNotificationCallback`）来监听默认音频端点变更事件，因此当用户切换默认音频设备后，`session_manager` 仍指向旧设备的会话枚举器，`set_process_volume` 对新设备上的 mpv 进程可能失效。

**修复 / 现状（v41-A-001，部分缓解）**：
- ✅ **方案 B 自愈重试已实现**：`with_session` 在 WASAPI 调用失败时自动触发 `refresh_session_manager` 重试一次（L199-206），`refresh_session_manager`（L129-170）已具备 COM 重新初始化 + 设备重连能力，缓存清理并允许后续重试。
- ⚠️ **主动监听仍未实现**：`volume.rs:13` 顶部仍保留 `// TODO(A-001): 未实现 IMMNotificationClient 主动监听`；`device_enumerator` 字段、`refresh_session_manager` 在设备切换瞬间仍无法主动感知（仅失败后惰性自愈）。
- 对应 v6.0 技术债 **A-TD-001**（死字段 + 主动监听未实现，见 v6 技术债清单）。

**建议**：
- 方案 A（推荐）：实现 `IMMNotificationClient`，在 `OnDefaultDeviceChanged` 回调中触发 `refresh_session_manager`，注意回调来自 WASAPI 线程，需通过 channel/poster 转发，避免回调中直接加锁死锁。
- 方案 B（已部分落地）：维持惰性自愈，同时按 A-TD-001 清理建议评估移除 `device_enumerator` 死字段或补齐主动监听。

#### [A-002] [Medium] [代码逻辑] volume.rs:161 — `with_session` 枚举路径 `GetSession`/`cast` 失败用 `?` 中止 ✅ 已修复于 v4.0 Wave 2H

**描述**：`with_session` 的会话枚举循环对三类 WASAPI 调用采取不一致的容错策略：`GetSession(i)` / `cast::<IAudioSessionControl2>()` 失败用 `?` 立即中止整个枚举，而 `GetProcessId()` 改为 `continue`（A01 修复）。实践中会话列表可能在枚举过程中变更，`GetSession`/`cast` 失败会丢失后续可能匹配的会话。

**修复状态**：✅ 已修复于 v4.0 Wave 2H（经代码核验 `volume.rs:316-329`：`GetSession(i)` 与 `control.cast::<IAudioSessionControl2>()` 均已改为 match + `continue` 容错，与 `GetProcessId()` 策略一致，并记录 `tracing::trace!` 日志）。

#### [A-003] [Medium] [可维护性] volume.rs（整体） — 核心 WASAPI 交互逻辑无任何集成测试覆盖 ✅ 已修复于 v4.0 Wave 2H

**描述**：v4.0 审查时 `volume.rs` 测试全部围绕 `new_disabled()`（降级 no-op 实例）或纯辅助函数展开，未覆盖 `with_session` cache hit/cache miss 路径与真实音频会话的端到端操作，此前修复无回归防护。

**修复状态**：✅ 已修复于 v4.0 Wave 2H；v4.1 追加真实 COM 交互测试（`v41_a003_real_com_cache_hit_miss_e2e`）。注意：核心 WASAPI 交互仍仅由 `#[ignore]` 真机测试覆盖（CI 无端到端验证，见 v6 §1.3 / A-TD-002）。

#### [A-004] [Low] [资源管理] volume.rs:52, 13-21 — `VolumeControl` 无显式 `Drop`，COM 接口 Release 跨 apartment 发生 ✅ 已修复于 v4.0 Wave 3D

**描述**：`VolumeControl` 无 `impl Drop`，依赖 `windows` crate 自动生成的 `Drop`。`unsafe impl Send` 后，最后一个 `Arc` clone 可能在任意线程被 drop（主线程 STA、tokio worker MTA、pause 线程 MTA），存在跨 apartment / 未初始化 COM 线程释放的隐患。

**修复状态**：✅ 已修复于 v4.0 Wave 3D（方案 ②：在 `unsafe impl Send` 上方 SAFETY 注释末尾追加"Drop 契约（A-004）"段落，文档化"不实现显式 `impl Drop`、依赖 COM 自动 `Release()`、最后一个 `Arc clone` 的 drop 必须发生在已初始化 COM 的线程"调用方契约；未新增 `impl Drop`）。（v6 关联：A-TD-003，契约段被复制到 `session_cache` 字段下造成重复）

#### [A-005] [Low] [错误处理] volume.rs:287 — `is_pid_running` 对 `WAIT_FAILED` 按「已退出」处理，可能误清有效缓存项 ✅ 已修复于 v4.0 Wave 3D

**描述**：`is_pid_running` 仅判断 `WAIT_TIMEOUT`（仍在运行），其余（含 `WAIT_OBJECT_0`、`WAIT_FAILED`）均返回 `false`。`WAIT_FAILED` 可能由瞬态错误触发，导致有效缓存项被误清。

**修复状态**：✅ 已修复于 v4.0 Wave 3D（采纳"追求更稳健"方案：导入 `WAIT_FAILED`，改为 match 表达式，`WAIT_FAILED` 分支记录 `tracing::trace!` 并返回 `true` 保守保留缓存项）。

#### [A-006] [Low] [可维护性] volume.rs:290-298 — `collect_stale_pids` 文档自相矛盾（「纯函数」 vs 「调用 Win32 API」） ✅ 已修复于 v4.0 Wave 3D

**描述**：`collect_stale_pids` 的文档注释前后矛盾。

**修复状态**：✅ 已修复于 v4.0 Wave 3D（删除"纯函数"表述，改为"此辅助函数从 ..."，保留"对 Win32 进程查询的薄封装，非纯函数"表述不变）。（v6 关联：A-TD-002，仍被视为冗余抽象）

### ipc 模块 findings（9 项）

#### [I-001] [High] [并发安全] client.rs:312-350, 380 — `read_line_with_timeout` 在部分行数据场景下 `fill_buf` 无限阻塞，超时机制失效 ✅ 已修复（方案 B：total_avail）

**描述**（v4.0 原始 finding）：`read_line_with_timeout` 在 `PeekNamedPipe` 报告有数据后将整行读取委托给 `read_line_with_limit`，后者在 BufReader 缓冲区耗尽后调用 `File::read()`。对 `PIPE_WAIT`（阻塞模式）管道，当 peek 的数据消费完毕但仍未遇到 `\n` 时，`fill_buf()` → `File::read()` 会无限阻塞，外层 deadline 检查永不执行，超时机制在「服务端发送部分行数据且保持管道打开」场景下完全失效。

**修复状态（代码核验，client.rs:300-475）**：✅ **已修复（方案 B：total_avail 限制）**：
- `read_line_with_timeout` 将 `read_line_with_limit` 读取量限制为 `total_avail = BufReader 缓冲残留 + PeekNamedPipe 报告的管道可用量`（L433-444），消费量达到 `total_avail` 仍未遇 `\n` 时主动返回当前已读取部分，由外层循环重新 peek + 累积，使 deadline 检查始终保持有效。
- `read_line_with_limit` 返回原始字节 `Vec<u8>` 而非 `String`；`v41-I-001` 在 `total_avail` 边界处追加 UTF-8 完整性检查（区分"截断"与"无效字节"，截断则继续 `fill_buf` 读取后续字节）。
- 配套测试：`i001_total_avail_zero_returns_empty_without_reading`、`test_read_line_with_limit_utf8_multibyte`、`test_read_line_with_limit_invalid_utf8_returns_raw_bytes` 等覆盖 total_avail 限制与 UTF-8 截断。

#### [I-002] [Medium] [错误处理] client.rs:384-387 — `PeekNamedPipe` 失败时错误被完全丢弃返回空字符串 ✅ 已修复于 v4.0 Wave 2H

**描述**：`read_line_with_timeout` 中 `PeekNamedPipe` 失败时错误被完全丢弃，返回 `Ok(String::new())`，调用方误判为「管道已关闭」，静默吞错。

**修复状态**：✅ 已修复于 v4.0 Wave 2H（代码核验 client.rs:470-478：区分 `ERROR_BROKEN_PIPE` / `ERROR_NO_DATA`（EOF 语义，返回空串）与其他真实错误，并记录日志）。

#### [I-003] [Medium] [并发安全] client.rs:191-195, 223 — `connect_named_pipe` 使用 `std::thread::sleep` 阻塞，文档约定无编译期保障 ✅ 已修复于 v4.0 Wave 2H

**描述**：`connect_named_pipe` 使用 `std::thread::sleep` 阻塞；整个 `NamedPipeClient` API 都是同步阻塞，需 `spawn_blocking` 包裹，但仅是文档约定，无编译期保障。

**修复状态**：✅ 已修复于 v4.0 Wave 2H（API 阻塞语义已在文档更醒目地标注，当前调用方均正确包裹 `spawn_blocking`）。长远看仍可考虑 async 封装，见 v6 优化机会。

#### [I-004] [Low] [可维护性] mpv_protocol.rs:14-19 — `MpvEvent` 结构体为死代码 ✅ 已修复于 v4.0 Wave 3E

**修复状态**：✅ 已修复于 v4.0 Wave 3E（方案 ①：保留 `MpvEvent`，添加 `#[allow(dead_code)]` 属性 + 预留注释说明用于未来 mpv 事件处理）。（v6 关联：I-TD-004，"未来扩展预留"至 v6.0 仍未使用）

#### [I-005] [Low] [安全] mpv_protocol.rs:166-178 — `set_volume(f32)`/`set_speed(f32)` 未做范围校验 ✅ 已修复于 v4.0 Wave 3E

**修复状态**：✅ 已修复于 v4.0 Wave 3E（代码核验 mpv_protocol.rs:164-166, 199-201：`set_volume`/`set_speed` 入口添加 `is_finite()` + `!(0.0..=100.0).contains(&volume)` / `!(0.25..=4.0).contains(&speed)` 校验，越界返回 `MirrorStarError::InvalidArgument`；新增 4 个单测 `set_volume_rejects_nan` / `set_volume_rejects_out_of_range` / `set_speed_rejects_nan` / `set_speed_rejects_out_of_range`）。

#### [I-006] [Low] [可维护性] client.rs:117, wp_proc.rs:12, mpv_protocol.rs:57,144 — 超时默认值分散且不一致 ✅ 已修复于 v4.0 Wave 3E

**修复状态**：✅ 已修复于 v4.0 Wave 3E（方案 ②：在 `crates/mirrorstar-core/src/ipc/mod.rs` 顶部追加模块级 `//!` 文档，含文件结构与超时对照表（`read_response_line` 5s / `PLAY_COMMAND_TIMEOUT` 15s / `send_command` 5s / `get_property` 2s）；未集中常量）。（v6 关联：I-TD-015，对照表在 `client.rs` 与 `ipc/mod.rs` 重复且部分失准）

#### [I-007] [Low] [资源管理] wp_proc.rs:261-265, mpv_protocol.rs:219-223 — `Drop` 双重 disconnect 冗余清理 ✅ 已修复于 v4.0 Wave 3E

**修复状态**：✅ 已修复于 v4.0 Wave 3E（方案 ②：保留外层 `Drop`，在上方追加注释说明 inner Drop 会再次调用 `disconnect()`，因 `Option::take()` 幂等故安全，保留外层 Drop 为记录 `tracing::info!` 断开日志）。（v6 关联：I-TD-008 / I-TD-012，注释承认冗余但保留，与 I-TD-002 一并评估）

#### [I-008] [Low] [可维护性] wp_proc.rs:163-171 — `send_command_no_wait` 文档未说明延迟响应副作用 ✅ 已修复于 v4.0 Wave 3E

**修复状态**：✅ 已修复于 v4.0 Wave 3E（`WpProcIpcClient::send_command_no_wait` 文档追加两段延迟响应副作用说明，与 mpv 侧 mpv_protocol.rs:124-126 一致）。

#### [I-009] [Low] [安全] wp_proc.rs:174, 211 — `play(source)`/`navigate(url)` 未在 IPC 层做 URL 协议白名单校验 ✅ 已修复于 v4.0 Wave 3E

**修复状态**：✅ 已修复于 v4.0 Wave 3E（方案 ②：在 `play`/`navigate` 文档注释追加"调用方契约"段落，明确 URL/源路径校验由上层负责，IPC 层作为薄封装不做防御性校验）。

### process 模块 findings（7 项）

#### [P-001] [High] [资源管理] manager.rs:82-85, 187-189 — `start()` 在前一进程已自行退出时跳过 `stop()`，直接覆盖句柄导致内核句柄泄漏 ✅ 已修复

**描述**（v4.0 原始 finding）：`start()` 清理前一进程逻辑 `if self.is_running() { self.stop()?; }`。当上一进程**自行退出**（mpv 播放结束、崩溃、被外部 kill）时 `is_running()` 返回 `false`，跳过 `stop()`，但 `process_handle`/`job_handle`/`pid` 仍持有旧值，随后用新句柄覆盖导致旧进程句柄 + 旧 Job Object 句柄泄漏（每重启一次泄漏 2 个内核句柄）。

**修复状态（代码核验，manager.rs:80-87）**：✅ **已修复**：`start()` 开头改为无条件清理旧句柄 —— `let had_old_handle = self.process_handle.is_some() || self.job_handle.is_some(); if had_old_handle { self.stop_immediate()?; }`。`stop_immediate`（v41-P-002）直接 `TerminateProcess` + `CloseHandle`，毫秒级完成，不再依赖 `is_running()` 分支，句柄泄漏路径已消除。（v6 关联：P-TD-001/P-TD-002，配套量化测试 `v41_p001_start_does_not_leak_handles` 标 `#[ignore]`，CI 永不执行）

#### [P-002] [Medium] [性能] manager.rs:370-371, 206-227 — `Drop` 调用 `stop()` 阻塞最长 8s，拖慢应用退出 ✅ 已修复于 v4.0 Wave 2H

**修复状态**：✅ 已修复于 v4.0 Wave 2H（`Drop` 改为调用 `stop_immediate()` 而非 `stop()`，避免 8s 阻塞；代码核验 manager.rs:440-444 `// P-002: Drop 中调用 stop_immediate 而非 stop`）。

#### [P-003] [Medium] [性能] manager.rs:83-85 — `start()` 检测到旧进程仍在运行时同步等待 8s 才启动新进程，壁纸切换延迟 ✅ 已修复于 v4.0 Wave 2H

**修复状态**：✅ 已修复于 v4.0 Wave 2H（`start()` 用 `stop_immediate()` 替代 `stop()`，切换场景立即终止旧进程，不再 8s 等待；代码核验 `start()` 无条件 `stop_immediate()`）。（v6 关联：P-TD-009，注释引用未实现的 `wait_for_exit(100ms)` 兜底，实际需调用方自行 sleep 100ms 等 OS 回收 PID）

#### [P-004] [Medium] [错误处理] manager.rs:255, 252-254, 371 — `wait_and_terminate` 始终返回 `Ok(())`，`Result` 返回类型误导且产生死代码分支 ✅ 已修复于 v4.0 Wave 2H

**修复状态**：✅ 已修复于 v4.0 Wave 2H（采用使返回类型与真实语义一致的方案，消除调用方误导性的 `if let Err` 死代码分支；`wait_and_terminate` 返回值语义对齐）。

#### [P-005] [Low] [安全] manager.rs:407-416 — `escape_windows_arg` 拒绝 `\n`/`\r` 但未拒绝 `\0`（NUL 截断命令行风险） ✅ 已修复于 v4.0 Wave 3F

**修复状态**：✅ 已修复于 v4.0 Wave 3F（代码核验 manager.rs:574：`if arg.contains('\n') || arg.contains('\r') || arg.contains('\0')`，拒绝 NUL；新增 `escape_windows_arg_rejects_nul` / `escape_windows_arg_rejects_nul_at_end` 测试）。

#### [P-006] [Low] [可维护性] manager.rs:116 — `CREATE_UNICODE_ENVIRONMENT` 标志在 `lpEnvironment=None` 时为无效标志 ✅ 已修复于 v4.0 Wave 3F

**修复状态**：✅ 已修复于 v4.0 Wave 3F（代码核验 manager.rs:11-12, 120-123：已移除 `CREATE_UNICODE_ENVIRONMENT`，`CreateProcessW` 调用 `dwCreationFlags` 为 `CREATE_NEW_PROCESS_GROUP | CREATE_NO_WINDOW`，并加注释说明 lpEnvironment=None 时该 flag 无效）。

#### [P-007] [Low] [错误处理] manager.rs:266-275, 291-303 — `wait_and_terminate` 的 WAIT_FAILED 路径对可能无效的句柄调用多个 API 产生日志噪声 ✅ 已修复于 v4.0 Wave 3F

**修复状态**：✅ 已修复于 v4.0 Wave 3F（代码核验 manager.rs:340-353：`WAIT_FAILED` 分支不再调用 `terminate_and_wait`，改为仅记录一条 `tracing::warn!`（含 `last_os_error()`），控制流直接落入末尾 `CloseHandle` 清理路径）。

---

## v6.0 技术债清单及清理状态（完整保留）

> 来源：v6.0 技术债审查（2026-07-25）audio/ipc/process 模块原始技术债清单，完整保留，含后续清理状态跟踪。
...
### 3.2 v4.0 优先修复项当前状态（交集）

v4.0 曾列出的 3 项 High 优先级修复，经代码核验：

1. **A-001 音频设备变更监听**：⚠️ **仍未完成**。`with_session` 失败时惰性 `refresh_session_manager` 方案 B 已实现（自愈），但 `IMMNotificationClient` 主动监听未实现（仍为 `TODO(A-001)`），建议继续按 A-TD-001 评估（实现主动监听或移除死字段）。
2. **I-001 超时机制理论盲区**：✅ **已修复**。通过 `total_avail` 参数限制 `fill_buf` 消费量（方案 B）+ v41-I-001 UTF-8 截断处理，超时机制在部分行数据场景下已生效。
3. **P-001 句柄泄漏**：✅ **已修复**。`start()` 开头无条件调用 `stop_immediate()` 清理旧句柄，句柄泄漏路径已消除。
...
### 3.4 交集汇总（v4.0/v5.0/v6.0 关联）

#### 3.4.1 v6 与 v4.0 findings 关联（引用本文档 v4 findings ID）
