...
**规范示例（非项目现存代码，仅作约定示范）**：`OwnedFileHandle` 为一个可转移所有权的 RAII 句柄封装，按下列签名轮廓实现（核心意图：`from_raw` 接管原生句柄所有权、`into_raw` 释放所有权、`Deref`/`DerefMut` 让现有调用方平滑迁移、`Drop` 时校验 null / `INVALID_HANDLE_VALUE` 后 `CloseHandle`）。此处仅保留命名与签名约定，方法体以实现细节以真实模块中 `OwnedProcHandle` / `OwnedHandle` 为准：

```rust
pub struct OwnedFileHandle { handle: HANDLE }

impl OwnedFileHandle {
    pub unsafe fn from_raw(handle: HANDLE) -> Self; // 接管所有权
    pub fn into_raw(self) -> HANDLE;                // 释放所有权
}

impl Deref for OwnedFileHandle { type Target = HANDLE; }        // + DerefMut
impl Drop for OwnedFileHandle { /* 仅留下方要点中的 Drop 契约 */ }
```
...
- `docs/05-优化文档/06-src-tauri应用层.md` — ST-004 Wave 2C 修复说明（`MirrorStarError::InvalidArgument` 引入）
- `docs/05-优化文档/05-audio-ipc-process模块.md` — IPC 错误处理与 `ResponseStatus::Error` 使用约定
