...
> ⚠️ **本文档已归档（2026-07-15）**
>
> 本文档已重构为按模块组织的结构化文档集。最新内容请查阅 [文档索引](../index.md)。
>
...
>
> **新文档结构**（均位于本目录，入口见 [文档索引](../index.md)）：
...
> - [附录A-已修复问题汇总.md](./11-附录A-已修复问题汇总.md) — v1.0→v3.5 已修复问题汇总
> - [附录B-版本历史.md](./12-附录B-版本历史.md) — v1.0→v3.5.3 版本变更日志
>
> 以下为原文档内容，保留作为历史记录，不再更新。

---

> ⚠️ **状态说明**：本文档为 v3.5 历史归档快照，正文中所有标注『待实施 / 步骤 n（v3.5）』的条目均已在后续 v3.5→v4.0→v4.1 轮修复完成，仅作历史记录，切勿视为当前待办。当前状态以 README 索引页及各模块文档（02~09）为准。
...
MirrorStar Wallpaper 的核心技术特性：**事件驱动模型**（SetWinEventHook + TaskbarCreated + WM_POWERBROADCAST）替代轮询，实现暂停态 CPU 0%；**双 IPC 协议**（mpv JSON + wp-proc 自定义 JSON）支撑主进程与渲染子进程通信；**Rust 核心**（配合 TypeScript 前端）借助所有权系统在编译期保证内存安全；**暂停态零开销**（内存 17.14MB、CPU 0%）；**混合进程模型**（主进程 + 按需子进程，无 watchdog）；**4 种壁纸类型**（GIF/视频/网页/图片）。
...
- `const WM_SPAWN_WORKER: u32 = 0x052C;`（注释"未文档化的 Progman 消息，触发 WorkerW 创建"）
...
### A.15 统一编排单一来源（unify-arrangement-single-source）

基于已批准 spec（change-id：unify-arrangement-single-source）完成（完成态），移除重复的排列配置，使编排事实来源唯一。

1. ✅ 移除 `AppConfig.display` 字段与 `DisplayConfig` 结构体（BREAKING 配置 schema）：`[display]` 表被 serde 自动忽略，旧配置向后兼容。
2. ✅ `rotation.arrangement` 成为编排唯一来源（per_monitor / all_same / span）：前端独立"排列模式"下拉已删除，仅保留轮换面板"轮换编排"下拉。
3. ✅ 编排变更同步引擎：`reconcile_and_align` 在编排变更时同步 `WallpaperEngine.arrangement`；引擎锁忙则 warn 跳过，下次 reconcile 补齐。
4. ✅ Span 行为审计（Task 4）：Span 实为"每屏渲染器窗口覆盖虚拟桌面"（N 渲染器、N 份解码，视觉近似跨屏）；"单跨屏渲染器、1 次 apply"为已知差距（详见 `10-壁纸轮换调度器-Rotation-Scheduler.md` §8.5）。

### A.16 多屏排列定位提升（promote-multiscreen-arrangement）

基于已批准 spec（change-id：promote-multiscreen-arrangement）完成（完成态），将编排设置从"轮换编排"定位提升为设置面板顶层的"多屏排列"分组，使文档命名与代码一致。

1. ✅ 编排设置提升为设置面板顶层"多屏排列"分组（此前为"轮换编排"、埋在"壁纸轮换"分组下）。
2. ✅ 命名按新定位优化：如 `rotationArrangementSelect`→`arrangementSelect` 等。
3. ✅ 数据仍存 `rotation.arrangement` 单一来源（per_monitor / all_same / span），**无 schema 变更**。
4. ✅ 编排链路技术债审查：全链路核对未发现确认死代码，本次为纯命名与文档定位升级。
...
10. **架构文档同步**：03-模块设计-Module-Design.md、05-依赖与数据流-Dependency-and-Data-Flow.md、07-暂停恢复机制-Pause-Resume.md、02-系统架构-System-Architecture.md、quality.md 中 lib.rs 行数、系统托盘位置（→state.rs）、全屏检测位置（→platform/fullscreen.rs）、wp-proc 行数等全部更正。
...
### A.17 布局域引入（introduce-layout-domain）

基于已批准 spec（change-id：introduce-layout-domain）完成（完成态），将多屏排列迁移为顶层 `AppConfig.arrangement` 并新增 `mirrorstar-core` 的 `layout/` 布局域模块，统一"编排 → 调度单元"映射。

1. ✅ 配置 schema（BREAKING）：多屏排列从 `[rotation] arrangement`（`RotationConfig.arrangement`）迁移为顶层 `AppConfig.arrangement`；旧配置经 serde legacy 字段（`arrangement_legacy`，`skip_serializing`）自动迁移，保存后旧键自动清理；`Arrangement` 枚举公共路径 `mirrorstar_core::Arrangement` 不变。
2. ✅ 新增 `mirrorstar-core` 的 `layout/` 布局域模块（DR-2）：`LayoutPlane`/`LayoutMonitor`/`plan()`/`unit_keys()`/`validate_unit_key()` 纯函数，负责"多屏排列 × 显示器拓扑 → 调度单元"映射，消除 `reconcile_and_align`/`commands::rotation::validate_unit_key`/前端 `unitKeysForArrangement` 三处重复映射（纯函数可脱离桌面环境单测）。
3. ✅ 引擎同步路径变更：`WallpaperEngine::set_arrangement` 不再由轮换调度器 `reconcile_and_align` 调用，改由通用配置路径（`update_config` / 新增 `update_arrangement` 命令 / 热更新回调）同步，消除"非轮换场景改排列不同步引擎"的缺口。
4. ✅ 前端：多屏排列选择器从轮换面板逻辑迁至独立 `ui/arrangement.ts`（顶层布局逻辑），select id 由 `rotation-arrangement-select` 改为 `arrangement-select`；新增 IPC 命令 `get_arrangement` / `update_arrangement`。
