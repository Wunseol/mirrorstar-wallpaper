...
MirrorStar Wallpaper 围绕以下特性设计：**4 种壁纸类型**（GIF/视频/网页/图片）；**事件驱动模型**，通过 SetWinEventHook、TaskbarCreated 与 WM_POWERBROADCAST 感知系统变化，而非轮询；**双 IPC 协议**（mpv JSON + wp-proc 自定义 JSON）承担主进程与渲染子进程通信；**Rust 核心**配合 TypeScript 前端，依靠所有权系统在编译期保证内存安全；**暂停态零开销**，暂停时内存约 17.14MB、CPU 0%；**混合进程模型**，主进程按需拉起子进程，无需常驻 watchdog。
